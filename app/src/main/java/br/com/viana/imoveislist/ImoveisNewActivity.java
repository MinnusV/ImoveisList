    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        switch (item.getItemId()){
            case R.id.StudentsInsert_MenuOk:
                ...
                try {
                    ImoveisDAO imoveisDAO = new ImoveisDAO(ImoveisNewActivity.this);
                    imovel = imoveisListUtils.buildImovelForInsert();

                    if (imovel.getId() != null){
                        imoveisDAO.update(imovel);
                    }else {
                        imoveisDAO.create(imovel);
                    }

                    imoveisDAO.close();

                    Toast.makeText(ImoveisNewActivity.this, "Novo Imovel " + imovel.getName() + " Salvo!", Toast.LENGTH_SHORT).show();
                    finish();
                } catch (Exception e) {
                    Toast.makeText(ImoveisNewActivity.this, "Erro ao salva o imovel. \n" + e.getMessage(), Toast.LENGTH_SHORT).show();
                    e.printStackTrace();
                }
                break;
        }
